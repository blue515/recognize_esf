...
void
estimate_esf(const boost::filesystem::path &path, pcl::PointCloud <pcl::ESFSignature640> &signature)
{
  ...
  */
  ...
  // Estimate esf signature of the point cloud
  
  //pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ> ());
  pcl::ESFEstimation<pcl::PointXYZ,pcl::ESFSignature640> esf;
  //esf.setRadiusSearch(0.06);
  esf.setInputCloud (cloud);
  //esf.setInputNormals (normals);
  //esf.setSearchMethod (tree);
  esf.compute (signature);
}
...
bool 
checkesf(const boost::filesystem::path &path)
{
  int esf_idx;
...
loadHist (const boost::filesystem::path &path, esf_model &esf)
{
  ...
  // Fill esf_model.second with histogram data
...
void
loadData(const boost::filesystem::path &base_dir, std::vector<esf_model> &models)
{
  ...
  else
  {
  	...
  	for (boost::filesystem::directory_iterator i (base_dir); i != boost::filesystem::directory_iterator (); i++)
	{
	  ...
	  if (boost::filesystem::is_regular_file (i->status ()) && boost::filesystem::extension (i->path ()) == ".pcd")
	  {
		  ...
		  if(!checkesf(str))
		  {
		  	str.replace(str.find(".pcd"), 4, "_esf.pcd");
		  	if(!boost::filesystem::exists(str))
		  	{
		  	pcl::PointCloud <pcl::ESFSignature640> signature;
		  	estimate_esf(i->path().string(), signature);
...
void
visualize(int argc, char** argv, int k, double thresh, std::vector<esf_model> models, flann::Matrix<int> k_indices, flann::Matrix<float> k_distances)
{
  // Load the results
  pcl::visualization::PCLVisualizer p (argc, argv, "esf Cluster Classifier");
  ...
  for (int i = 0; i < k; ++i)
  {
    ...
    boost::replace_last (cloud_name, "_esf", "");
...
inline void
nearestKSearch (flann::Index<flann::ChiSquareDistance<float> > &index, const esf_model &model, 
